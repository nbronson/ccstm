 ...
 * @since 0.1
 */
public class Benchmark implements org.multiverse.benchmark.Benchmark {
	...
	public org.multiverse.benchmark.BenchmarkThread createThread(int i, int nb) {
		return new BenchmarkThread(i, nb, m_accounts, m_max, m_read_frequency, m_write_frequency, m_read_threads, m_write_threads);
	}

	public String getStats(org.multiverse.benchmark.BenchmarkThread[] threads) {
		int total = 0;
		...
		return "TOTAL: T=" + transfers + ", R=" + reads + ", W=" + writes + "\n\nTOTAL=" + total + sb.toString();
	}
}
