...
object Perf {
  ...
  def main(args: Array[String]) {
    ...
    val workerClass = (try {
      ...
      case _ => Class.forName("edu.stanford.ppl.ccstm.experimental.perf." + args(args.length - 1))
  ...
  def exitUsage {                             
    println("scala edu.stanford.ppl.ccstm.experimental.perf.Perf [options] map_type TestClass\n" +
            "Options:\n" +
            "  --threads=N (default 1)\n" +
            "  --warmup=N (default 10)\n" +
            "  --passes=N (default 10)\n" +
