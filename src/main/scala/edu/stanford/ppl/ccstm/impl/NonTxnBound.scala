...
private[ccstm] class NonTxnBound[T](val unbind: Ref[T],
  ...
  def weakCompareAndSet(before: T, after: T): Boolean = compareAndSet(before, after)

  def weakCompareAndSetIdentity[R <: AnyRef with T](before: R, after: T): Boolean = compareAndSetIdentity(before, after)

  def transform(f: T => T) { getAndTransform(f) }
