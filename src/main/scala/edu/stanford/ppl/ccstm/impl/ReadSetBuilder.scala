...
private[impl] final class ReadSetBuilder {
  ...
  private var _versions = new Array[STMImpl.Version](maxSizeForCap(InitialCap) + 1)
  ...
  def add(handle: Handle[_], version: STMImpl.Version): Unit = {
    val slot = STMImpl.hash(handle.ref, handle.offset) & (_dispatch.length - 1)
    addImpl(slot, _dispatch(slot), handle, version)
  }

  @tailrec
  private def addImpl(slot: Int, i: Int, handle: Handle[_], version: STMImpl.Version): Unit = {
    if (i == 0) {
  ...
  private def append(slot: Int, handle: Handle[_], version: STMImpl.Version): Unit = {
    val i = _size + 1
    _size = i
    _handles(i - 1) = handle
    ...
    _dispatch(slot) = i
    if (_size > maxSizeForCap(_dispatch.length)) grow()
  }
  ...
  private def grow(): Unit = {
    ...
    _versions = new Array[STMImpl.Version](maxSizeForCap(c) + 1)
    ...
    while (i < s) {
      val h = hh(i)
      append(STMImpl.hash(h.ref, h.offset) & (c - 1), h, vv(i))
      i += 1
    }
  }
  ...
  def result(): ReadSet = {
    _dispatch = null
    _next = null
    new ReadSet(_size, 0, _handles, _versions)
  }
}