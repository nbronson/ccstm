...
# Usage: exp_multiverse.sh subdir
  ...
  for t in 1 2 4 8 16 32 64 128 256; do 
    for f in alpha; do 
      for cont in low high; do 
        ...
        out=$m/pass$pass/multiverse-cont_$cont-$t.out
        if grep 'Nb iterations' $out >/dev/null 2>&1; then
          echo "$out already present and valid, skipping"
        else
          ../../../../bin/proj_java --multiverse org.deuce.benchmark.Driver -n $t -w 20000 org.deuce.benchmark.bank_mult.Benchmark -n $a -i 100000  > $out
        fi
      done
    done
  done
