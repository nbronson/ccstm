...
In an additional departure from typical STM designs, CCSTM's transactions
use a hybrid of static and dynamic binding.  The current transaction
context is passed to \type{Ref} instances via an implicit parameter,
avoiding the performance penalty that would result from evaluating a
dynamic binding during each access.  Nesting of transactions, however,
is resolved dynamically, allowing composition without requiring each
intervening method to declare an implicit \type{Txn} parameter.
