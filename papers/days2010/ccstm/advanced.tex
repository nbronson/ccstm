...
A CCSTM \type{Ref} provides both a value that names a specific memory
...
area some methods are not mirrored.  Every access operation in \type{Ref} is
present in \type{Ref.Bound}.  All of the methods are defined
for both transactional and non-transactional contexts, even if they are
mainly useful only for one of those. 
\type{Source.Bound} declares methods that
read, \type{Sink.Bound} declares methods that write, and \type{Ref.Bound}
...
\code{(}\type{Txn}\code{ => }\typeparam{Z}\code{)} to an \type{AtomicBlock} that provides a
