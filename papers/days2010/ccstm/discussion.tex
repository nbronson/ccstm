...
\item \code{elem} -- A pair of methods \code{elem} and \code{elem\_=}
could be used to provide the illusion that transactional data was actually
stored in a variable of \type{Ref}.  This approach is self-consistent,
...
hindered by the escape action semantics of \code{nonTxn}.  The advanced
...
\subsection{\keyword{\bfseries @specialized}}

The \keyword{@specialized} annotation has the potential to reduce
...
boxing by using separate implementations of \type{Ref} for each primitive
(selecting using a \type{ClassManifest} during creation if the type is
not statically known), but boxing occurs during the reused portions of
the barrier code.  The current implementation of \keyword{@specialized}
interacts non-trivially with the manual specialization of \type{Ref}
