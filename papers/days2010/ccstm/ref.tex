...
moved inside of \code{map}, however, then no rollback is required:
\lstset{numbers=none}
\begin{lstlisting}
  if (x.map(_ > 100)) { ... }
\end{lstlisting}
...
\item \code{apply(): T} -- Equivalent to \code{get}.

\item \code{get : T} -- Reads the value managed by the bound \type{Ref}.
...
reevaluating \code{f} to avoid rollbacks (\code{f} must be idempotent).

\item \code{await(p: T => Boolean)} -- Blocks until \code{p(get)} is true.
Transactional contexts block using \code{retry}, the modular blocking
primitive.  Non-transactional contexts just block the current thread.

\item \code{unrecordedRead: UnrecordedRead[T]} -- Returns an instance
that wraps the value that would be returned by \code{get}, but does not
add anything to the transaction's read set (if any).

\item \code{releasableRead: ReleasableRead[T]} -- Reads the value managed by
the bound \type{Ref}, and returns that value in an instance that allows the
...
\item \code{:=(v: T)} -- Equivalent to \code{set(v)}.

\item \code{set(v: T)} -- Updates the value managed by the bound \code{Ref}. 
If this view is bound to a non-transactional context, this method will
linearize the store before returning.

\item \code{tryWrite(v: T): Boolean} -- Immediately performs an update and
...
\item \code{readForWrite: T} -- Returns the same value as that returned
by \code{get}, but adds the bound \code{Ref} to the write set of the
transaction context, if any.

\item \code{getAndSet(v: T): T} -- Atomically invokes \code{set(v)}
...
\item \code{weakCompareAndSet(b: T, v: T): Boolean} -- Works like \code{compareAndSet}, but allows spurious failures.
